...
const BestSellersCarousel: React.FC = () => {
  ...
  return (
    <div className="mt-2 lg:mt-5 md:w-[78%] xl:w-[67%] text-center mx-auto overflow-hidden rounded-xl">
      <div className="mx-auto p-8">
        <h2 className="md:flex lg:flex text-start mb-8 text-2xl font-bold">
          Os livros mais vendidos
        </h2>
        <Slider {...settings}>
          {products.filter((item) => item.carouselCategory === "bestSellersCarousel").map((item) => (
            <Link
              key={item.id}

  ...
  return (
    ...
    >
      <div className="mx-auto p-8 pb-10">
        ...
        <Slider {...settings}>
          {products.filter(item => item.category === "eletrodomesticos")
            .map(item => (
              <Link
                key={item.id}
                href={`/products/${item.id}`}
                className="rounded-2xltext-center cursor-pointer translate hover:scale-95">
                ...
                <p className="mt-3 lg:mt-10 mb-3 truncate ml-5">
                  {item.name}
                </p>
                ...
                <span className="absolute">
                  {(!handleRatingCarousel(item.id) || handleRatingCarousel(item.id) <= 0) ?
                    <>
                      ...
                      <>
                        <a className='text-xs ml-1 text-sky-700 cursor-pointer'>
                ...
                <p className="mt-3 lg:text-lg font-extrabold">

    ...
    return (
        <div>
            ...
            <div className='my-10 mx-52'>
                <h1 className='text-3xl text-gray-600 font-semibold my-8'>Todos os livros</h1>
                <div className='grid grid-cols-3 gap-20'>
                    {products.filter((item) => item.category === "livros").map((item) => (
                        <a key={item.id} href={`/products/${item.id}`} className="cursor-pointer translate hover:scale-95 border p-2">
                            <img src={item.image} className="h-36 lg:h-56 lg:mt-4 mx-auto rounded-md object-contain" />
                            <p className="mt-3 lg:mt-10 lg:mb-3">{item.name}</p>
